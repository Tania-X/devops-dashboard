...
### 9.5 二期规划：角色权限配置页面（已实施，2026-08-12）
...
| 项 | 设计要点（实施结果） |
|----|----------|
| 后端 | `GET /api/settings/roles`、`GET /api/settings/permissions`、`PUT /api/settings/roles/:role`（仅 admin，权限点 `settings:manage`）；写操作执行 `RemoveFilteredPolicy` + `AddPolicies` + `LoadPolicy()` 热生效，无需重新登录 |
| 前端 | 设置页新增"角色权限"Tab（`settings:manage` 权限控制显隐）：角色 × 权限点分组矩阵，勾选保存（AntD Checkbox），admin 锁定不可改 |
| 权限点 | 复用权限点常量清单（`obj:act`），前端矩阵按分组渲染，中文标签在 `permissionLabels.ts` |
| 前置 | Step 2（RequirePermission 全量接入）完成；路由标注已改用 `authz` 权限点常量（§9.6 TODO 落地） |
| 状态 | **已实施**（2026-08-12），含单元测试 + API 冒烟验证热生效 |

### 9.6 权限事实源决策（TODO，2026-08-10 用户提出）

**现象**：权限点信息散落多处——前端（菜单 `perm`、`AuthButton perm`、角色下拉 options）与后端（`authz` 常量、路由标注、`rolePolicies()`）各自维护同一套字符串，拼写不一致会静默失效（菜单不显示 / 接口 403）。

**决策：两类事实源分开存放，不一律入库。**

| 信息 | 类型 | 存放位置 | 理由 |
|------|------|----------|------|
| 权限点定义（`user:read` 等） | 代码契约 | 后端 `authz` 包常量（保持现状） | 编译期引用、可验证；入库则编译期无法校验、改码需同步改库，收益低 |
| 角色→权限映射 | 运行时数据 | 数据库 `casbin_rule` 表（二期 §9.5 做配置页） | 需动态配置 + 热生效（`Reload()`） |

**TODO（缓解前端重复维护）**：
- [x] 前端菜单/按钮 `perm` 统一引用后端 `/auth/me` 返回的权限点（菜单过滤已是此模式）；角色下拉 options 由后端返回（`GET /api/settings/roles` 已落地，2026-08-12）
- [x] 后端路由标注与 `authz` 常量同源（`RequirePermission` 改为接收权限点常量，2026-08-12）
- 明确**不做**：权限点定义入库（`9.5` 配置页只管理"角色↔权限映射"，权限点清单仍是代码常量）
