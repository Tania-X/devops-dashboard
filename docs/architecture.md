...
**TODO（缓解前端重复维护）**：
- [ ] 前端菜单/按钮 `perm` 统一引用后端 `/auth/me` 返回的权限点（当前菜单过滤已是此模式），角色下拉 options 由后端返回（后续可加 `GET /api/settings/roles` 或复用权限点接口）
- [ ] 后端路由标注与 `authz` 常量同源（用权限点常量枚举替代裸字符串 `"user","read"`，避免手写漂移）
