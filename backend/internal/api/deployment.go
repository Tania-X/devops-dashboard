...
// @Tags        Deployment
// @Success     200 {array} deploymentdomain.Deployment
...
// @Success     200 {array} deploymentdomain.DeploymentHistory
...
func (h *Handler) GetDeploymentHistory(c *gin.Context) {
	...
	historyList, err := h.services.DeploymentService.GetDeploymentHistory(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
